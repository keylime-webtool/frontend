...
export function Integrations() {
  const viewMode = useVisualizationStore((s) => s.integrationsViewMode);
  const setViewMode = useVisualizationStore((s) => s.setIntegrationsViewMode);
  ...
  const serviceList = useMemo(
    () => (Array.isArray(services) ? services : []) as IntegrationService[],
    [services],
  );
        ...
        >
          ...
          <button
            ...
            style={{
              padding: '5px 14px',
              fontSize: '12px',
              fontWeight: viewMode === 'topology' ? 600 : 400,
              background: viewMode === 'topology' ? 'var(--color-primary)' : 'var(--color-surface)',
              color: viewMode === 'topology' ? 'white' : 'var(--color-text-secondary)',
              border: 'none',
              cursor: 'pointer',
            }}
          >
        ...
        <>
          ...
          <div className="section" style={{ opacity: backendUp ? 1 : 0.5 }}>
            ...
            {!backendUp ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
                {serviceList.map((svc: IntegrationService) => (
                  <div
                    key={svc.name}
                    style={{
                      border: '1px solid var(--color-border)',
